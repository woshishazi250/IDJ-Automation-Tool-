...
class BlockItem(QGraphicsRectItem):
    ...
    def setup_block_visual(self):
        ...
        #                                      -                                           Scratch                ?        self.setRect(0, 0, width, height)
        self.setBrush(QBrush(block_color))
        self.setPen(QPen(QColor(0, 0, 0), 2))
        self.setAcceptedMouseButtons(Qt.LeftButton)
        
        #                         
        self.text_item = QGraphicsTextItem(self.block_name, self)
        self.text_item.setPos(15, 15 if self.block_type in ["loop", "if", "function"] else 12)
        self.text_item.setDefaultTextColor(Qt.white)
        self.text_item.setFont(QFont("Arial", 11, QFont.Bold))
        
        #                                                  -                         
        from PyQt5.QtWidgets import QGraphicsEllipseItem
        self.top_connection = QGraphicsEllipseItem(width // 2 - 8, -4, 16, 8, self)
        self.top_connection.setBrush(QBrush(block_color))
        self.top_connection.setPen(QPen(QColor(0, 0, 0), 2))
        
        #                                                  -                         
        if self.block_type in ["loop", "if", "function"]:
            #                                                                             ?            self.bottom_connection = QGraphicsEllipseItem(width // 2 - 8, height - 4, 16, 8, self)
        else:
            self.bottom_connection = QGraphicsEllipseItem(width // 2 - 8, height - 4, 16, 8, self)
        self.bottom_connection.setBrush(QBrush(block_color))
        self.bottom_connection.setPen(QPen(QColor(0, 0, 0), 2))
        
        #                                                     ?        self.input_connection = QGraphicsRectItem(width - 10, height // 2 - 3, 6, 6, self)
        self.input_connection.setBrush(QBrush(Qt.yellow))
        self.input_connection.setPen(QPen(Qt.black, 1))
        ...
        #                                                                                     
        #             QGraphicsDropShadowEffect                                                                                                                                                                      ?        #                                                                   QGraphicsRectItem                                          QGraphicsDropShadowEffect

        
    def get_block_color(self, block_type):
        """                                                            """
        color_map = {
            "loop": QColor(120, 180, 0),
            "if": QColor(255, 180, 0),
            "function": QColor(100, 150, 255),
            "wait": QColor(0, 150, 255),
            "delay": QColor(0, 150, 255),

            "mouse_move": QColor(255, 100, 100),
            "mouse_click": QColor(255, 100, 100),
            "mouse_drag": QColor(255, 100, 100),
            "keyboard_input": QColor(255, 150, 100),
            "find_image": QColor(150, 100, 200),
            "find_text": QColor(150, 100, 200),
            "image_match": QColor(150, 100, 200),
            "and": QColor(100, 200, 100),
            "or": QColor(100, 200, 100),
            "not": QColor(100, 200, 100),
            "jump": QColor(200, 100, 200)
        }
        
        return color_map.get(block_type, QColor(150, 150, 150))
    ...
    def _collect_child_offsets(self):
        ...
        #                                                                                                                                     
        if hasattr(self, 'connected_blocks'):
            for child in self.connected_blocks:
                offset = child.pos() - self.pos()
                self.child_offsets[child] = offset
                #                                                                             ?                child._collect_child_offsets_recursive(self, offset)
                
        #                                                                                                                         
        if hasattr(self, 'connected_from') and self.connected_from:
            offset = self.connected_from.pos() - self.pos()
            self.child_offsets[self.connected_from] = offset
            #                                                                                                 
            self.connected_from._collect_child_offsets_recursive(self, offset)
            
    def _collect_child_offsets_recursive(self, root_block, parent_offset, processed=None):
        """                                                                                        ?""
        ...
        #                                                     ?        if hasattr(self, 'connected_blocks'):
            for child in self.connected_blocks:
                offset = child.pos() - root_block.pos()
                root_block.child_offsets[child] = offset
                child._collect_child_offsets_recursive(root_block, offset, processed)
                
        #                                                                                                                                                                 ?        if hasattr(self, 'connected_from') and self.connected_from:
            offset = self.connected_from.pos() - root_block.pos()
            root_block.child_offsets[self.connected_from] = offset
            self.connected_from._collect_child_offsets_recursive(root_block, offset, processed)
    ...
    def mouseMoveEvent(self, event):
        ...
        if self.isSelected() and event.buttons() == Qt.LeftButton and hasattr(self, 'drag_start_pos'):
            ...
            if self.is_dragging:
                ...
                #                                                           ?                for child, offset in self.child_offsets.items():
                    child.setPos(new_pos + offset)
                    
                #                                                                         
                self._update_drag_visual_feedback()
                
                #                                                                                                                                   ?                self.check_disconnections()
                
                #                                                                                                                                                     ?                #                                                                                                                       ?                #                               _collect_child_offsets                                                            
    ...
    def _apply_boundary_constraints(self, pos):
        ...
        #                                                                                                                                                     ?        #                             ?BlockEditor                                         ?(0, 0, 450, 500)
        scene_rect = QRectF(0, 0, 450, 500)
        block_rect = self.sceneBoundingRect()
        
        #                                                                                                                         
        constrained_pos = QPointF(
            max(0, min(pos.x(), scene_rect.width() - block_rect.width())),
            max(0, min(pos.y(), scene_rect.height() - block_rect.height()))
        )
    ...
    def _update_drag_visual_feedback(self):
        ...
        #                                               ?        for child in self.child_offsets.keys():
    ...
    def check_disconnections(self):
        ...
        #                                                                                               ?        if self.is_connected:
            #                                                                                                           ?            if self.connected_to:
                self_top_pos = self.pos() + QPointF(self.rect().width() // 2, 0)
                other_bottom_pos = self.connected_to.pos() + QPointF(self.connected_to.rect().width() // 2, self.connected_to.rect().height())
                
                distance = (self_top_pos - other_bottom_pos).manhattanLength()
                
                #                                                                         13                ?                if distance > 13:
                    other_name = self.connected_to.block_name  #                             ?                    self.disconnect_from(self.connected_to)
                    logger.info(f"             {self.block_name}     ?{other_name}                                                                         ")
            
            #                                                                                                           ?            if self.connected_from:
                self_bottom_pos = self.pos() + QPointF(self.rect().width() // 2, self.rect().height())
                other_top_pos = self.connected_from.pos() + QPointF(self.connected_from.rect().width() // 2, 0)
                
                distance = (self_bottom_pos - other_top_pos).manhattanLength()
                
                #                                                                         13                ?                if distance > 13:
                    other_name = self.connected_from.block_name  #                             ?                    self.disconnect_from(self.connected_from)
                    logger.info(f"             {self.block_name}     ?{other_name}                                                                         ")
            
            #                                                                                                                               
            if hasattr(self, 'connected_blocks') and self.connected_blocks:
                for child in self.connected_blocks.copy():  #             copy()                                                    ?                    if self.block_type in ["loop", "if", "function"] and hasattr(self, 'contain_area'):
                        ...
                        #                                                                                                                 ?0%                ?5%    ?                        if overlap_ratio < 0.15:
                            child_name = child.block_name  #                             ?                            self.disconnect_from(child)
                            logger.info(f"             {child_name}     ?{self.block_name}                                         ? {overlap_ratio:.2f}    ?)
            ...
            if self.connected_to and self.connected_to.block_type in ["loop", "if", "function"]:
                ...
                if overlap_ratio < 0.15:
                    parent_name = self.connected_to.block_name  #                             ?                    self.disconnect_from(self.connected_to)
                    logger.info(f"             {self.block_name}     ?{parent_name}                                         ? {overlap_ratio:.2f}    ?)
    ...
    def check_connections(self):
        ...
        def distance_to_other(item):
            self_center = self.pos() + QPointF(self.rect().width() // 2, self.rect().height() // 2)
            item_center = item.pos() + QPointF(item.rect().width() // 2, item.rect().height() // 2)
            return (self_center - item_center).manhattanLength()
            
        all_blocks.sort(key=distance_to_other)
        
        for item in all_blocks:
            logger.debug(f"                      ?{self.block_name}     ?{item.block_name}                 ?)
            
            #                                                                                                                         
            #                                                                                           A                  B      B                                          A    ?            if self.can_connect_to(item) and not (item.connected_to == self or item.connected_from == self):
                logger.debug(f"             {self.block_name}                             ?{item.block_name}")
    ...
    def can_connect_to(self, other_block):
        ...
        #                                                                                                                                                 
        self_top_pos = self.pos() + QPointF(self.rect().width() // 2, 0)
        other_bottom_pos = other_block.pos() + QPointF(other_block.rect().width() // 2, other_block.rect().height())
        
        distance = (self_top_pos - other_bottom_pos).manhattanLength()
        
        #                                                                                                                                                 
        horizontal_alignment = abs((self.pos().x() + self.rect().width() // 2) - 
                                 (other_block.pos().x() + other_block.rect().width() // 2)) < 60
        
        #                                                                         self                         other_block                 ?        vertical_direction = (self.pos().y() > other_block.pos().y())
        
        #                                                                                               ?        vertical_connection = distance < 80 and horizontal_alignment and vertical_direction
        
        if vertical_connection:
            logger.debug(f"             {self.block_name}                                         ?{other_block.block_name} "
                        f"(            : {distance:.1f}px,                         : {horizontal_alignment},             : {vertical_direction})")
            return True
    ...
    def connect_to(self, other_block):
        ...
        distance = (self_top_pos - other_bottom_pos).manhattanLength()
        horizontal_alignment = abs((self.pos().x() + self.rect().width() // 2) - 
                                 (other_block.pos().x() + other_block.rect().width() // 2)) < 150
        vertical_direction = (self.pos().y() > other_block.pos().y())
        
        is_vertical_connection = distance < 180 and horizontal_alignment and vertical_direction
        ...
        if is_vertical_connection:
            ...
            if self.connected_to:
                logger.debug(f"                                                             {self.connected_to.block_name}                 ?)
                self.connected_to.disconnect_from(self)
                
            #                                                 self                        other_block                ?            self.connected_to = other_block
            other_block.connected_from = self
        
            #                                                                                                             
            # self                  other_block                ?            target_x = other_block.pos().x() + (other_block.rect().width() - self.rect().width()) // 2
            target_y = other_block.pos().y() + other_block.rect().height() + 5
            
            #                                                                                                                       ?            if abs(self.pos().x() - target_x) > 5 or abs(self.pos().y() - target_y) > 5:
                self.setPos(target_x, target_y)
        
            self.is_connected = True
            other_block.is_connected = True
        
            logger.info(f"             {self.block_name}                             ?{other_block.block_name}      {self.block_name}                        {other_block.block_name}                        ")
        
            #                                                                 ?            self.highlight_connection()
            other_block.highlight_connection()
            return
...
class BlockEditor(QWidget):
    ...
    def setup_ui(self):
        ...
        self.block_library.setHeaderLabel("                ?)
        ...
        #                                                                                                                 ?        self.script_scene.setSceneRect(0, 0, 450, 500)
        self.script_scene.setBackgroundBrush(QBrush(QColor(240, 240, 240)))
        
        self.script_view = QGraphicsView(self.script_scene)
        self.script_view.setFixedWidth(450)
        self.script_view.setFixedHeight(500)  #                                     
        self.script_view.setDragMode(QGraphicsView.NoDrag)
        ...
        #                                                                         
        self.script_view.setSceneRect(0, 0, 450, 500)
        #                                                             
        ...
        property_group = QGroupBox("                      ?)
        property_layout = QVBoxLayout(property_group)
        
        self.block_type_label = QLabel("                        :")
    ...
    def init_blocks(self):
        """                                    """
        #                         
        control_group = QTreeWidgetItem(self.block_library, ["                        "])
        QTreeWidgetItem(control_group, ["            ", "loop"])
        QTreeWidgetItem(control_group, ["                        ", "if"])
        QTreeWidgetItem(control_group, ["                ?, "function"])
        QTreeWidgetItem(control_group, ["            ", "wait"])
        QTreeWidgetItem(control_group, ["            ", "jump"])
        
        #                         
        logic_group = QTreeWidgetItem(self.block_library, ["                        "])
        QTreeWidgetItem(logic_group, ["    ?, "and"])
        QTreeWidgetItem(logic_group, ["    ?, "or"])
        QTreeWidgetItem(logic_group, ["    ?, "not"])
        
        #                         
        action_group = QTreeWidgetItem(self.block_library, ["                        "])
        QTreeWidgetItem(action_group, ["                        ", "mouse_move"])
        QTreeWidgetItem(action_group, ["                        ", "mouse_click"])
        QTreeWidgetItem(action_group, ["                        ", "mouse_drag"])
        QTreeWidgetItem(action_group, ["                        ", "keyboard_input"])
        
        #                                     
        image_group = QTreeWidgetItem(self.block_library, ["                                    "])
        QTreeWidgetItem(image_group, ["            ", "find_image"])
        QTreeWidgetItem(image_group, ["            ", "find_text"])
        QTreeWidgetItem(image_group, ["                        ", "image_match"])
